...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (plant) => {
    ...
    dispatch(addItem({ ...plant, cost: numericCost, quantity: 1 }));
  };
            ...
            <>
              ...
              <div className='product-list'>
                {category.plants.map((plant, plantIndex) => {
                  const id = `${category.category}-${plant.name}`.toLowerCase().replace(/\s+/g, '-'); // Genera un ID único
                  const plantWithId = { ...plant, id };
                  ...
                  return (
                    <div className='product-card' key={plantIndex}>
                      ...
                      <button
                        ...
                        onClick={() => handleAddToCart(plantWithId)}
                        disabled={cartItems.some((item) => item.id === id)}
                      >
                        {cartItems.some((item) => item.id === id) ? 'Added to Cart' : 'Add to Cart'}
                      </button>
                    </div>
                  );
                })}
